...
public class ForApp06 {
	...
	public static void main(String[] args) {
		/*
		 * break와 continue (반복문 안에서만 사용가능)
		 *  - break와 continue는 반복문 내에만 사용할 수 있는 키워드다
		 *  	(break는 switch에서 사용가능하다)
		 *  - break는 이 키워드가 포함된 가장 안쪽 반복문을 탈출한다
		 *  - continue는 반복이 진행중에 continue를 만나면 
		 *    반복문의 끝(for는 증감식, while은 조건식으로 이동)으로 이동해서 
		 *    다음 번 반복으로 넘어간다
		 * 
		 *  예) 
		 *  	for (초기식; 조건식; 증감식) {
		...
		// 1 ~ 100 까지 순서대로 저장해서 합계를 구할 때 합계가 1000을 돌파하는 순간의
		// 합계와 그 순간의 숫자값을 출력하기
		int sum = 0;
		int number = 0;
